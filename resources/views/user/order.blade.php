...
@section('content')
<section>
    <div class="main-title">Chi tiết đơn hàng {{ $transaction->id }}</div>
    <div class="row">
        <div class="col-4">
    ...
    <div class="content">
        ...
        <div>
            <a href="{{ route('get.user.transaction') }}" class="btn btn-light"><i class="fa fa-angle-double-left"></i>
                Quay lại đơn hàng</a>
            @if ($transaction->tst_status != -1)
                <a href="{{ route('get.user.tracking_order', $transaction->id) }}" class="btn btn-primary js-">Theo dõi đơn
                    hàng <i class="fa fa-angle-double-right"></i></a>
            @endif
        </div>
    </div>
</section>
